...
const T: React.CSSProperties = { display: 'block', transform: 'scaleY(-1)' };
    ...
    >
      <div style={{ maxWidth: '480px', margin: '0 auto' }}>
        ...
        {navRoute && (
          <div style={{ padding: '8px 16px 0' }}>
            <div className="bg-[var(--surface2)] rounded-xl px-3 py-2">
              <span className="text-[var(--text)] text-sm font-medium truncate" style={T}>
                {navRoute.name}
              </span>
            </div>
          </div>
        ...
        <div style={{ position: 'relative', display: 'flex', alignItems: 'center', padding: '4px 16px 8px' }}>
          ...
          >
            <div style={{ fontSize: '10px', color: '#888', marginBottom: '4px', ...T }}>{subLabel}</div>
            <div style={{ fontSize: '16px', fontWeight: '700', ...T }}>{subSpeed.toFixed(1)}</div>
            <div style={{ fontSize: '10px', color: '#888', ...T }}>km/h</div>
          </div>
          ...
          >
            <div style={{ fontSize: '10px', color: '#888', marginBottom: '4px', ...T }}>
              {showRemaining ? '残り' : '走行距離'}
            </div>
            <div style={{ fontSize: '16px', fontWeight: '700', ...T }}>
              {showRemaining ? remainingKm.toFixed(2) : rideKm.toFixed(2)}
            </div>
            <div style={{ fontSize: '10px', color: '#888', ...T }}>km</div>
          </div>
        </div>
