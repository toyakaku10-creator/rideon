...
export default function Home() {
  ...
  const startDemoRide = (pts: [number, number][]) => {
    ...
    const cumTotal = cumDist[cumDist.length - 1]
    // スケール係数：cumDistをtotalDistanceに合わせる
    const scale = totalDistance / cumTotal
    ...
    const animate = (now: number) => {
      ...
      const targetDist = (totalDistance * progress) / scale  // cumDistスケールに変換
      ...
      // 走行距離はtotalDistanceベースで正確に
      setRideDistance(totalDistance * progress)
